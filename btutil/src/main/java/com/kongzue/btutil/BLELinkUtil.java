...
public class BLELinkUtil {
    ...
    //开启蓝牙设备
    public void openBluetooth(BluetoothOpenListener openListener) {
        if (!openBt()) {
            //启动蓝牙失败
            loge("启动蓝牙失败");
            if (openListener != null) openListener.onResponse(false, ERROR_START_BT, "启动蓝牙失败");
        } else {
            //启动蓝牙成功
    ...
    private void doCheckBtOpened(BluetoothOpenListener openListener) {
    ...
    public boolean startGetNotification(String serviceUUID,String characteristicUUID,OnBLENotificationListener listener) {
        ...
        if (service != null) {
            ...
            if (descriptor != null) {
                ...
                loge("descriptor不存在："+serviceUUID.toString());
            }
            return bluetoothGatt.setCharacteristicNotification(characteristic, true);
        } else {
